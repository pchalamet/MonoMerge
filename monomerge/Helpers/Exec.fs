...
let private defaultPSI (command : string) (arguments: string) (dir : DirectoryInfo) (vars : Map<string, string>) redirect =
...
let private supervisedExec redirect (command : string) (arguments: string) (dir : DirectoryInfo) (vars : Map<string, string>) =
    printfn $"Exec: {command} {arguments}"
    let psi = defaultPSI command arguments dir vars redirect
    use proc = Process.Start (psi)
    if proc |> isNull then failwithf "Failed to start command %A" command
    ...
    | [| MonitorCommand.End code; MonitorCommand.Out out; MonitorCommand.Err err |]
            -> { Result.Code=code
                 Result.Out=out
                 Result.Error=err
                 Result.Info = sprintf "%s @ %s" command dir.FullName }
